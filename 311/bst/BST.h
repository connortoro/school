...
public:
  ...
  Node(const T& e){right = left = NULL; el = e;};
...
int BST<T>::getEvenNodes(Node<T>* p) //private function. WHY?
{
  ...
  int toAdd = 0;
  if(p->el % 2 == 0) toAdd = 1;
  return getEvenNodes(p->left) + getEvenNodes(p->right) + toAdd;
}
...
bool BST<T>::remove(const T& e) //public function
{
  ...
  else if(parent->el > e) //parent's left child to be removed
    remove(parent->left);
...
void BST<T>::remove(Node<T>*& p) //private function
{
  ...
  //If the node to be deleted has 2 children
      ...
      while(temp->right !=NULL)
	{
      ...
      if(t_parent == p) //the largest value is the direct left child of the node whose value was replaced by the largest (Diagram 4-one child or no children)
