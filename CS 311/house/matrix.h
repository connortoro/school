...
ostream& operator<<(ostream& o, const Matrix<T>& mrx)
{
  ...
  */
  for(int row = 0; row < mrx.R; row++) {
    for(int col = 0; col < mrx.C; col++) {
      cout << setw(10) << mrx.m[row][col];
    }
    cout << endl;
  }
   return o;
...
Matrix<T> Matrix<T>::operator*(const Matrix<T>& other)
{
 ...
 if(C != other.R) {
  throw Matrix<T>::size_error();
 }

 Matrix<T> multipliedMatrix(R, other.C);

 //for each element in new matrix
 for(int row = 0; row < R; row++) {
  for (int col = 0; col < other.C; col++) {

    //Calculate cross product and place result in new Matrix<T>
    T crossProduct = 0;
    for(int i = 0; i < C; i++) {
      crossProduct += m[row][i] * other.m[i][col];
    }
    multipliedMatrix.m[row][col] = crossProduct;
  }
 }
